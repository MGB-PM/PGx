...
def _diplotypes_to_phenotypes(genotyping_report, gene_annotations_dir, genes_without_annotations):
	gene_phenotypes = {}
	...
	for gene in all_genes: 
		...
		with open(genotypes_phenotypes_fname, 'r') as f:
			...
			for line in f:
				...
				if dict(zip(variant_ids_to_match, zygosities)).items() <= called_variant_zygosities.items():
		...
		for genegroup in druggroups[druggroup]:
			if genegroup in gene_drug_pgx_annotations:
				...
				if druggroup not in outputs:
					outputs[druggroup] = []
				outputs[druggroup].append([genegroup,diplotype,metabolism])
			else:
